...
from dataloaders.acdc import  ISICDataset_DIFF
from networks.net_factory import net_factory
from utils import losses, val_2d


parser = argparse.ArgumentParser()
parser.add_argument('--root_path', type=str, default='/home/user/ISIC2017', help='Name of Experiment')
parser.add_argument('--dataset', type=str, default = 'ISIC2017', help='the dataset used in the experiments')

parser.add_argument('--labeled-id-path', type=str, default='/home/user/ISIC2017/1%_labeled_3.txt')
parser.add_argument('--unlabeled-id-path', type=str, default='/home/user/ISIC2017/1%_unlabeled_3.txt')

parser.add_argument('--exp', type=str, default=r'DDC_NoPreRE3H5', help='experiment_name')
...
def self_train(args, snapshot_path):
    ...
    trainset_u = ISICDataset_DIFF(args.dataset, args.root_path, 'train_u', 
                             args.patch_size[0], args.unlabeled_id_path)
    trainset_l = ISICDataset_DIFF(args.dataset, args.root_path, 'train_l', 
                             args.patch_size[0],args.labeled_id_path, nsample=len(trainset_u.ids))
    
    db_val = ISICDataset_DIFF(args.dataset, args.root_path, "val")
    ...
    for _ in iterator:
        ...
        for i,((img_x, mask_x),
            ...
            logging.info('iteration %d: loss: %f, loss_unsup: %f, loss_fp: %f, loss_con: %f'%(iter_num, loss, loss_unsup, loss_fp, loss_con))
