...
class ISICRandomGenerator(object):
    def __init__(self, output_size):
        self.output_size = output_size
    ...
    def __call__(self, sample):
        ...
        image, label = image.resize((self.output_size[0], self.output_size[1]), Image.BICUBIC), label.resize((self.output_size[0], self.output_size[1]), Image.NEAREST)
        #image, label = normalize(image, label)
        sample = {'image': image, 'label': label}
        return sample

class ISICRandomGenerator_h5(object):
    def __init__(self, output_size):
        self.output_size = output_size
...
class LAHeart(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None):
        ...
        train_path = self._base_dir[:-13]+'train.list'
        test_path = self._base_dir[:-13]+'test.list'
