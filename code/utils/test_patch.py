...
def var_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA"):
    if dataset_name == "LA":
        ...
        image_list = ["/home/user/SSL4MIS-master/data/LA/processed_h5/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    elif dataset_name == "Pancreas_CT":
        with open('/home/user/Pancreas-CT-all/test.list', 'r') as f:
            image_list = f.readlines()
        image_list = ["/home/user/Pancreas-CT-all/" + item.replace('\n', '') + "/mri_norm2.h5" for item in image_list]
    elif dataset_name == 'BraTS':
        with open('/home/user/SSL4MIS-master/data/BraTS2019/val.txt', 'r') as f:
            image_list = f.readlines()
        image_list = ["/home/user/SSL4MIS-master/data/BraTS2019/" + item.replace('\n', '') + ".h5" for item in image_list]
...
def test_all_case(model_name, num_outputs, model, image_list, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, save_result=True, test_save_path=None, preproc_fn=None, metric_detail=1, nms=0):
    ...
    for image_path in loader:
        h5f = h5py.File(image_path, 'r')
        image = h5f['image'][:]
        if stride_xy == 18 and stride_z == 4:
            label = h5f['label_full'][:]
        elif stride_xy == 16 and stride_z ==16:
            label = h5f['label'][:]
